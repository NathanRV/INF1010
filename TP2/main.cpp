...
1.	Quel est l’utilité de l’opérateur = et du constructeur par copie?
	L'opérateur = sert à copier les attributs d'un objet sur un objet qui existe déjà tandis
	que le constructeur par copie fait la même chose, mais avec un objet qui n'existe pas auparavant.

2.	Dans quel cas est-il nécessaire de surcharger l’opérateur = et le constructeur par copie? 
	Lorsque la classe contient des attributs qui sont des pointeurs ou lorsque l'on ne veut 
	pas copier tous les attributs.
	Sans surcharge, le constructeur par défaut copie simplement les attributs, cela résulte en une
	"shallow copy", c'est-à-dire que l'on peut se retrouver avec des pointeurs qui pointent au même 
	endroit.
...
4.	Qu’est-ce qui différencie l’opérateur = du constructeur par copie?
	L'opérateur = nécessite que l'objet ait déjà été initialisé tandis que le 
	constructeur par copie va initialiser l'objet en copiant l'autre.
*/
