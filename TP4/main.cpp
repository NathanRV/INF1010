...
1.	Nous remarquons que le classe Solde à une méthode sans implémentation, getPrixBase(). Dans le contexte du polymorphisme, 
	...
	Cela permet de créer une classe abstraite, classe qui ne sert qu'à passer des variables et des méthodes à des classes enfants
	qui héritent de celle-ci. Ce sont des classes que l'on ne désire pas instancier, mais que l'on veut hériter de celle-ci. 
	C'est-à-dire qu'aucune objet ne peut appartenir direcetemt à cette classe.

2.	Pour quelle raison devons-nous déclarer certains destructeurs virtuels? 

	Toutes les classes de base polymorphe devrait avoir un destructeur virtuel afin de s'assurer que tous les destructeurs 
	de classe dont l'objet hérite puissent être appellés. De cette manière, il n'y a pas de risque que le mauvais destructeur
	soit appellé et qu'il y ait une fuite de mémoire.
